...
import SaudiComparison from "./components/SaudiComparison";
...
export default function BudgetCalculator() {
  ...
  const barData = useMemo(() =>
    EXPENSE_CATEGORIES.map((cat) => ({
      name: cat.name.split("و")[0].trim(), amount: expenses[cat.id], color: cat.color,
    })), [expenses]);
  ...
  return (
    <div className="space-y-6" dir="rtl">
      ...
      {/* Income Slider */}
      <IncomeSlider income={monthlyIncome} onChange={handleIncomeChange} />

      {/* Summary Cards */}
      <SummaryCards income={monthlyIncome} totalExpenses={totalExpenses} totalSavings={totalSavings} />

      {/* Expenses + Donut side by side on desktop */}
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* Expense Table */}
        <div className="lg:col-span-3 bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-bold text-gray-800 dark:text-white">توزيع المصروفات</h3>
            <div className="flex gap-2">
              ...
              >
                <Wand2 className="h-3.5 w-3.5" /> توزيع تلقائي ذكي
              </button>
              <button
                onClick={resetAll}
        ...
        {/* Donut + Saudi Comparison */}
        <div className="lg:col-span-2 space-y-6">
          <ExpenseDonutChart data={pieData} savingsRate={savingsRate} />
          <SaudiComparison expenses={expenses} income={monthlyIncome} />
        </div>
      </div>
      ...
      {/* Savings Projection */}
      <SavingsProjection monthlySavings={totalSavings > 0 ? totalSavings : 0} />
    </div>
  );
}
