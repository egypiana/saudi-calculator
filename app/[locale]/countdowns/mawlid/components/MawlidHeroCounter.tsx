...
interface MawlidHeroCounterProps {
  targetDate: string;
  year: number;
  hijri: string;
}
...
export default function MawlidHeroCounter({
  targetDate,
  year,
  hijri,
}: MawlidHeroCounterProps) {
  ...
  const handleShare = async () => {
    const text = `العد التنازلي للمولد النبوي الشريف ${year} - باقي ${timeLeft.days} يوم و ${timeLeft.hours} ساعة`;
    if (navigator.share) {
      try {
        await navigator.share({ title: `المولد النبوي الشريف ${year}`, text });
      } catch {
        // user cancelled
      }
    }
  ...
  const handleCopy = async () => {
    const text = `العد التنازلي للمولد النبوي الشريف ${year}\nباقي ${timeLeft.days} يوم و ${timeLeft.hours} ساعة و ${timeLeft.minutes} دقيقة\nالمولد النبوي: ${formatArabicDate(targetDate)}\n١٢ ربيع الأول ${hijri}`;
    try {
  ...
  return (
    <section
      dir="rtl"
      className="relative min-h-[70vh] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-teal-600 via-teal-700 to-emerald-800 rounded-3xl px-6 py-16 md:py-20"
    >
      {/* Animated Particles */}
      {particles.map((p) => (
        <span
          key={p.id}
          className="mawlid-particle absolute rounded-full bg-teal-300 opacity-50"
          style={{
            top: p.top,
            left: p.left,
            width: p.size,
      ...
      <div className="relative mb-8">
        <span className="text-6xl md:text-7xl drop-shadow-[0_0_20px_rgba(13,148,136,0.5)]">
          🕌
        </span>
      </div>
      ...
      <h1 className="text-2xl md:text-4xl font-bold text-white text-center mb-3">
        الوقت المتبقي حتى المولد النبوي الشريف {year}
      </h1>

      {/* Subtitle */}
      <p className="text-white/60 text-sm md:text-base text-center mb-4">
        {hijri} &middot; ١٢ ربيع الأول &middot; {formatArabicDate(targetDate)}
      </p>
      ...
      <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white/80 text-sm mb-10">
        <span>🕌</span>
        <span>إجازة رسمية في المملكة</span>
      </div>
      ...
      <div className="w-full max-w-xl mb-10">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "١٢ ربيع الأول", hijriLabel: "تاريخ المولد", date: formatArabicDate(targetDate).replace(/ \d{4}$/, ""), icon: "🕌", highlight: true },
            { label: "إجازة رسمية", hijriLabel: "في المملكة", date: "يوم واحد", icon: "🏛️" },
            { label: "ذكرى مولده ﷺ", hijriLabel: "خير البشر", date: "صلوا عليه", icon: "🌟" },
          ].map((item) => (
            <div
              key={item.label}
            ...
            >
              ...
              <span className={`text-xs font-bold mb-1 ${
                item.highlight ? "text-teal-300" : "text-white/80"
              }`}>
                {item.label}
              </span>
