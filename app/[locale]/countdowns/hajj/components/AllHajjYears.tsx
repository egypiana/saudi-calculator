...
export default function AllHajjYears({ locale }: { locale: string }) {
  ...
  const preview = HAJJ_YEARS.slice(0, 8);

  return (
    <section>
      <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4 flex items-center gap-2">
        <span>🗓️</span> استكشف جميع مواسم الحج
      </h2>
      ...
      <div className="mt-5 text-center">
        <Link
          href={`/${locale}/countdowns/hajj/years`}
          className="inline-flex items-center gap-2 px-8 py-3.5 bg-emerald-700 text-white rounded-xl font-bold text-sm hover:bg-emerald-800 transition-all shadow-lg"
        >
