...
interface UpcomingLailatulQadrProps {
  locale: string;
}

export default function UpcomingLailatulQadr({ locale }: UpcomingLailatulQadrProps) {
  const now = new Date();
  const todayStr = now.toISOString().slice(0, 10);

  const futureDates = LAILATUL_QADR_LIST.filter((d) => d.night29Date >= todayStr).slice(0, 5);
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-100 dark:border-gray-700/50 last:border-b-0 transition-colors hover:bg-gray-50 dark:hover:bg-white/5 ${
                      isNearest ? "bg-violet-50/50 dark:bg-violet-900/10" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-3.5 text-gray-600 dark:text-gray-300">
                      {formatArabicDate(entry.night27Date)}
                    </td>
                    <td className="px-4 py-3.5 text-gray-600 dark:text-gray-300">
                      {formatArabicDate(entry.lastTenStart)}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <Link
                        ...
                        className="text-violet-600 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300 text-xs font-medium transition-colors"
                      >
                        عرض التفاصيل
