...
export default function EidYearPage({ year, locale }: Props) {
  const data = EID_FITR_DATA[year];
  if (!data) return null;

  const yearIndex = EID_YEARS.indexOf(year);
  const prevYear = yearIndex > 0 ? EID_YEARS[yearIndex - 1] : null;
  const nextYear = yearIndex < EID_YEARS.length - 1 ? EID_YEARS[yearIndex + 1] : null;
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-dark-bg">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8" dir="rtl">
        <Breadcrumb items={[
          ...
          { labelAr: "جميع السنوات", labelEn: "All Years", href: "/countdowns/eid-fitr/years" },
          { labelAr: `عيد الفطر ${year}`, labelEn: `Eid ${year}` },
        ]} />

        <div className="mt-6 grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-6">
          <div className="space-y-8">
            <ShareButtons title={`كم باقي على عيد الفطر ${year}؟`} />

            <AdSlot id={`eid-${year}-top`} size="leaderboard" />
            ...
            {data.content.preparations.length > 0 && (
              <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-800/30 rounded-2xl p-5">
                <h3 className="font-bold text-green-800 dark:text-green-300 mb-3 flex items-center gap-2">
                  <span>🎯</span> نصائح الاستعداد لعيد {toAr(year)}
                </h3>
            ...
            <AdSlot id={`eid-${year}-mid`} size="rectangle" />
              ...
              {prevYear ? (
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="font-bold text-gray-800 dark:text-white">عيد الفطر {toAr(prevYear)}</div>
                  </div>
                </Link>
              ...
              {nextYear ? (
                ...
                >
                  <div className="text-left">
                    ...
                    <div className="font-bold text-gray-800 dark:text-white">عيد الفطر {toAr(nextYear)}</div>
                  </div>
                  <span className="text-lg">←</span>
                </Link>
              ) : <div className="flex-1" />}
            ...
            <AdSlot id={`eid-${year}-bottom`} size="leaderboard" />
          </div>
...
/* ─── Hero ─── */
function YearHero({ data }: { data: EidFitrYearData }) {
  const [time, setTime] = useState(getTimeUntilEid(data.year));

  useEffect(() => {
    const interval = setInterval(() => setTime(getTimeUntilEid(data.year)), 1000);
  ...
  return (
    <div className={`relative overflow-hidden ${
      isPast
        ...
        : "bg-gradient-to-br from-[#0a1628] via-[#1a0a2e] to-[#0f2a1a]"
    }`}>
      {!isPast && (
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 text-center" dir="rtl">
        <div className="mb-2">
          <span className="inline-block bg-white/10 backdrop-blur-sm border border-white/15 rounded-full px-4 py-1.5 text-sm text-yellow-200">
            {data.content.heroSubtitle}
          </span>
        </div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2">
          {isPast ? "عيد الفطر" : isCurrent ? "🎊 عيد الفطر" : "العد التنازلي لعيد الفطر"} {toAr(data.year)}
        </h1>
        ...
        {isPast ? (
          <div className="countdown-glass inline-block px-8 py-4">
            <p className="text-white/80 text-lg">انتهى عيد الفطر {toAr(data.year)}</p>
            <p className="text-white/50 text-sm mt-1">رمضان {toAr(data.ramadanDays)} يوماً</p>
          </div>
        ) : isCurrent ? (
          <div className="countdown-glass inline-block px-8 py-4">
            <p className="text-yellow-300 text-xl font-bold">🎊 عيد مبارك!</p>
      ...
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-l from-transparent via-yellow-400 to-transparent" />
    </div>
  );
}

/* ─── Year Info ─── */
function YearInfoBlock({ data }: { data: EidFitrYearData }) {
  const SEASON_ICONS: Record<string, string> = { "شتاء": "❄️", "ربيع": "🌸", "صيف": "☀️", "خريف": "🍂" };

  const items = [
    { icon: "🎊", label: "تاريخ عيد الفطر", value: `${formatDateAr(data.date)} — ${data.dayOfWeek}` },
    { icon: "🌙", label: "بداية رمضان", value: formatDateAr(data.ramadanStart) },
    { icon: "⏳", label: "أيام رمضان", value: `${toAr(data.ramadanDays)} يوم` },
  ...
  return (
    <section className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
      <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">
        بيانات عيد الفطر {toAr(data.year)}
      </h2>
...
/* ─── Year FAQ ─── */
function YearFAQ({ data, year }: { data: EidFitrYearData; year: number }) {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  const baseFAQ = [
    { q: `متى عيد الفطر ${toAr(year)}؟`, a: `عيد الفطر ${toAr(year)} يوم ${data.dayOfWeek} ${formatDateAr(data.date)}، الموافق ${data.hijriMonth} ${toAr(data.hijriYear)} هـ.` },
    { q: `كم يوم رمضان ${toAr(year)}؟`, a: `رمضان ${toAr(data.hijriYear)} هـ يستمر ${toAr(data.ramadanDays)} يوماً.` },
    { q: `في أي فصل عيد الفطر ${toAr(year)}؟`, a: `عيد الفطر ${toAr(year)} في فصل ${data.season}.` },
  ];
  ...
  return (
    <section>
      ...
      <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">
        أسئلة شائعة عن عيد الفطر {toAr(year)}
      </h2>
