...
import { EID_FITR_DATA, getNextEidYear, getTimeUntilEid, formatDateAr } from "@/lib/data/eidFitrData";

function toAr(num: number): string {
  return num.toString().replace(/\d/g, (d) => "٠١٢٣٤٥٦٧٨٩"[parseInt(d)]);
}

export default function EidHeroCounter() {
  const nextYear = getNextEidYear();
  const eidData = EID_FITR_DATA[nextYear];
  const [time, setTime] = useState(getTimeUntilEid(nextYear));

  useEffect(() => {
    const interval = setInterval(() => setTime(getTimeUntilEid(nextYear)), 1000);
  ...
  return (
    <section className="relative min-h-[75vh] sm:min-h-[80vh] flex flex-col items-center justify-center bg-gradient-to-br from-[#0a1628] via-[#1a0a2e] to-[#0f2a1a] overflow-hidden px-4 py-12 sm:py-16">
      {/* Stars */}
      ...
      <div className="relative z-10 text-center w-full max-w-2xl mx-auto" dir="rtl">
        ...
        <h1 className="text-white text-3xl sm:text-4xl lg:text-5xl font-bold mb-3">
          كم باقي على عيد الفطر المبارك؟
        </h1>

        {/* Date info */}
        <p className="text-yellow-300 text-lg sm:text-xl font-semibold mb-1">
        ...
        {/* Unique fact badge */}
        <div className="mb-7 px-4 py-2 bg-yellow-500/15 border border-yellow-500/30 rounded-full text-yellow-300 text-xs sm:text-sm text-center max-w-md mx-auto inline-block">
        ...
        <div className="w-full max-w-lg mx-auto mb-6">
          ...
          <div className="h-2 bg-white/10 rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-l from-yellow-400 to-green-500 rounded-full transition-all duration-1000"
        ...
        <div className="flex gap-3 justify-center">
          <button
            onClick={() => {
              if (navigator.share) {
                navigator.share({ title: `كم باقي على عيد الفطر ${nextYear}؟`, url: window.location.href });
              }
            }}
