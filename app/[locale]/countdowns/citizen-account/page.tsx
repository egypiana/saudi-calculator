...
export async function generateMetadata({ params: { locale } }: { params: { locale: string } }): Promise<Metadata> {
  ...
  const title = isAr ? "كم باقي على حساب المواطن؟ | موعد الصرف" : "Citizen Account Payment Countdown";
  const description = isAr ? "عداد تنازلي لموعد صرف دعم حساب المواطن." : "Countdown to Citizen Account (Hafiz) payment.";
  return {
    title,
    description,
    ...generatePageSEO(locale, "/countdowns/citizen-account", { title, description }),
  };
}
