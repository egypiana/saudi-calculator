...
import { ARAFA_DAY_DATA, ARAFA_DAY_YEARS, formatDateAr, type ArafaDayYearData } from "@/lib/data/arafaDayData";
...
const STATUS_LABELS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  future: { label: "\u0645\u0633\u062A\u0642\u0628\u0644\u064A", color: "text-orange-500", bg: "bg-orange-50 dark:bg-orange-900/20" },
};
...
export default function ArafahYearsClient({ locale }: { locale: string }) {
  ...
  const allYears = ARAFA_DAY_YEARS.map((y) => ARAFA_DAY_DATA[y]);
  ...
  const nextArafaDay = allYears.find((d) => d.status === "upcoming" || d.status === "current");

  return (
    <div dir="rtl">
      {/* Hero Stats */}
      <div className="bg-gradient-to-bl from-amber-600 via-orange-700 to-orange-800 rounded-2xl p-6 sm:p-8 mb-8 text-white">
        <h2 className="text-2xl sm:text-3xl font-bold mb-2">
          مواعيد يوم عرفة من 2025 إلى 2050
        </h2>
        <p className="text-amber-100 mb-6 text-sm sm:text-base">
          {toArabicDigits(totalYears)} عاماً من مواعيد يوم عرفة الدقيقة مع تواريخ عيد الأضحى والحج
        </p>
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-white/10 rounded-xl p-4 text-center backdrop-blur-sm">
            <div className="text-2xl sm:text-3xl font-bold">{toArabicDigits(totalYears)}</div>
            <div className="text-amber-100 text-xs sm:text-sm mt-1">إجمالي السنوات</div>
          </div>
          <div className="bg-white/10 rounded-xl p-4 text-center backdrop-blur-sm">
            <div className="text-2xl sm:text-3xl font-bold">{toArabicDigits(pastCount)}</div>
            <div className="text-amber-100 text-xs sm:text-sm mt-1">سنوات ماضية</div>
          </div>
          <div className="bg-white/10 rounded-xl p-4 text-center backdrop-blur-sm">
            <div className="text-2xl sm:text-3xl font-bold">{toArabicDigits(futureCount)}</div>
            <div className="text-amber-100 text-xs sm:text-sm mt-1">سنوات قادمة</div>
          </div>
        </div>
        {nextArafaDay && (
          <div className="mt-4 bg-white/10 rounded-xl p-4 backdrop-blur-sm text-center">
            <span className="text-amber-100 text-sm">أقرب يوم عرفة: </span>
            <span className="font-bold text-lg">٩ ذو الحجة {toArabicDigits(nextArafaDay.year)}</span>
            <span className="text-amber-100 text-sm mx-2">—</span>
            <span className="text-sm">
              باقي {toArabicDigits(Math.max(0, getDaysUntil(nextArafaDay.arafaDate)))} يوم
            </span>
          </div>
        )}
      </div>
      ...
      <div className="mt-12 bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/30 rounded-2xl p-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4 text-sm text-amber-700 dark:text-amber-300/80">
          <div className="flex gap-2">
            <span>🕋</span>
            <p>يوم عرفة هو ركن الحج الأعظم — قال النبي ﷺ: الحج عرفة</p>
          </div>
          <div className="flex gap-2">
            <span>📿</span>
            <p>صيام يوم عرفة يكفّر ذنوب سنتين — السنة الماضية والقادمة</p>
          </div>
          <div className="flex gap-2">
            <span>🤲</span>
            <p>خير الدعاء دعاء يوم عرفة: لا إله إلا الله وحده لا شريك له</p>
          </div>
          <div className="flex gap-2">
            <span>🌅</span>
            <p>فيه نزلت آية إكمال الدين: اليوم أكملت لكم دينكم وأتممت عليكم نعمتي</p>
          </div>
        </div>
      </div>
    </div>
...
function YearCard({ data, locale }: { data: ArafaDayYearData; locale: string }) {
  const status = STATUS_LABELS[data.status] || STATUS_LABELS.future;
  const daysUntil = getDaysUntil(data.arafaDate);
  const isPast = data.status === "past";

  return (
    <Link
      href={lp(locale, `/countdowns/arafah/${data.year}`)}
      ...
      <div className="p-4">
        ...
        <div className="space-y-1.5 text-xs text-gray-600 dark:text-gray-300 mb-3">
          <div className="flex items-center gap-2">
            ...
            <span>عرفة: {formatDateAr(data.arafaDate)}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-gray-400">🐑</span>
            <span>العيد: {formatDateAr(data.eidAdhaDate)}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="text-gray-400">📅</span>
        ...
        {!isPast && daysUntil > 0 ? (
          <div className="text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded-lg px-3 py-1.5 inline-block">
