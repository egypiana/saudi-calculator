...
interface UpcomingArafahProps {
  locale: string;
}

export default function UpcomingArafah({ locale }: UpcomingArafahProps) {
  const now = new Date();
  const todayStr = now.toISOString().slice(0, 10);

  const futureDates = ARAFA_DAY_LIST.filter((d) => d.arafaDate >= todayStr).slice(0, 5);
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-100 dark:border-gray-700/50 last:border-b-0 transition-colors hover:bg-gray-50 dark:hover:bg-white/5 ${
                      isNearest ? "bg-amber-50/50 dark:bg-amber-900/10" : ""
                    }`}
                  >
                    ...
                    <td className="px-4 py-3.5 text-gray-600 dark:text-gray-300">
                      {formatArabicDate(entry.arafaDate)}
                    </td>
                    <td className="px-4 py-3.5 text-gray-600 dark:text-gray-300">
                      {formatArabicDate(entry.eidAdhaDate)}
                    </td>
                    ...
                    <td className="px-4 py-3.5">
                      <Link
                        ...
                        className="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 text-xs font-medium transition-colors"
                      >
                        عرض التفاصيل
