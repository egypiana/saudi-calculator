...
interface AshuraHeroCounterProps {
  targetDate: string;
  year: number;
  hijri: string;
  tasuaDate: string;
...
export default function AshuraHeroCounter({
  targetDate,
  year,
  hijri,
  tasuaDate,
  eleventhDate,
}: AshuraHeroCounterProps) {
  ...
  const handleShare = async () => {
    const text = `العد التنازلي ليوم عاشوراء ${year} - باقي ${timeLeft.days} يوم و ${timeLeft.hours} ساعة`;
    if (navigator.share) {
      try {
        await navigator.share({ title: `يوم عاشوراء ${year}`, text });
      } catch {
        // user cancelled
      }
    }
  ...
  const handleCopy = async () => {
    const text = `العد التنازلي ليوم عاشوراء ${year}\nباقي ${timeLeft.days} يوم و ${timeLeft.hours} ساعة و ${timeLeft.minutes} دقيقة\nعاشوراء: ${formatArabicDate(targetDate)}\nتاسوعاء: ${formatArabicDate(tasuaDate)}`;
    try {
  ...
  return (
    <section
      dir="rtl"
      className="relative min-h-[70vh] flex flex-col items-center justify-center overflow-hidden bg-gradient-to-br from-emerald-700 via-teal-800 to-emerald-900 rounded-3xl px-6 py-16 md:py-20"
    >
      {/* Animated Particles */}
      {particles.map((p) => (
        <span
          key={p.id}
          className="ashura-particle absolute rounded-full bg-emerald-300 opacity-50"
          style={{
            top: p.top,
            left: p.left,
            width: p.size,
      ...
      <div className="relative mb-8">
        <span className="text-6xl md:text-7xl drop-shadow-[0_0_20px_rgba(16,185,129,0.5)]">
          📿
        </span>
      </div>
      ...
      <h1 className="text-2xl md:text-4xl font-bold text-white text-center mb-3">
        الوقت المتبقي حتى يوم عاشوراء {year}
      </h1>

      {/* Subtitle */}
      <p className="text-white/60 text-sm md:text-base text-center mb-4">
        {hijri} &middot; ١٠ محرم &middot; {formatArabicDate(targetDate)}
      </p>
      ...
      <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full text-white/80 text-sm mb-10">
        <span>📿</span>
        <span>
          صيام تاسوعاء وعاشوراء والحادي عشر &middot; {formatArabicDate(tasuaDate)} - {formatArabicDate(eleventhDate)}
        </span>
      </div>
      ...
      <div className="w-full max-w-xl mb-10">
        ...
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "تاسوعاء", hijriLabel: "٩ محرم", date: formatArabicDate(tasuaDate).replace(/ \d{4}$/, ""), icon: "🌙" },
            { label: "عاشوراء", hijriLabel: "١٠ محرم", date: formatArabicDate(targetDate).replace(/ \d{4}$/, ""), icon: "📿", highlight: true },
            { label: "الحادي عشر", hijriLabel: "١١ محرم", date: formatArabicDate(eleventhDate).replace(/ \d{4}$/, ""), icon: "🌟" },
          ].map((item) => (
            <div
              key={item.label}
            ...
            >
              ...
              <span className={`text-xs font-bold mb-1 ${
                item.highlight ? "text-emerald-300" : "text-white/80"
              }`}>
                {item.label}
              </span>
              <span className="text-[10px] text-white/50 mb-0.5">{item.hijriLabel}</span>
