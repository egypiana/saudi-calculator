...
import { ASHURA_DATA, ASHURA_YEARS, formatDateAr, getTimeUntilAshura, type AshuraYearData } from "@/lib/data/ashuraData";
...
export default function AshuraYearPage({ year, locale }: Props) {
  const data = ASHURA_DATA[year];
  if (!data) return null;

  const yearIndex = ASHURA_YEARS.indexOf(year);
  const prevYear = yearIndex > 0 ? ASHURA_YEARS[yearIndex - 1] : null;
  const nextYear = yearIndex < ASHURA_YEARS.length - 1 ? ASHURA_YEARS[yearIndex + 1] : null;
    ...
    <main className="min-h-screen bg-gray-50 dark:bg-dark-bg">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8" dir="rtl">
        <Breadcrumb items={[
          ...
          { labelAr: "جميع السنوات", labelEn: "All Years", href: "/countdowns/ashura/years" },
          { labelAr: `يوم عاشوراء ${year}`, labelEn: `Ashura Day ${year}` },
        ]} />

        <div className="mt-6 grid grid-cols-1 lg:grid-cols-[3fr_1fr] gap-6">
          {/* Main Content */}
          <div className="space-y-8">
            <ShareButtons title={`كم باقي على يوم عاشوراء ${year}؟`} />

            <AdSlot id={`ashura-${year}-top`} size="leaderboard" />
            ...
            <section className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
              <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">
                يوم عاشوراء {toArabicDigits(data.year)}
              </h2>
            ...
            <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/30 rounded-2xl p-5">
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="font-bold text-emerald-800 dark:text-emerald-300 mb-1">نصيحة للعبادة</h3>
                  <p className="text-sm text-emerald-700 dark:text-emerald-300/80">{data.content.worshipTip}</p>
                </div>
              </div>
            </div>
            ...
            <AdSlot id={`ashura-${year}-mid`} size="rectangle" />
            ...
            <div className="flex items-center justify-between gap-4">
              {prevYear ? (
                <Link
                  href={lp(locale, `/countdowns/ashura/${prevYear}`)}
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="font-bold text-gray-800 dark:text-white">يوم عاشوراء {toArabicDigits(prevYear)}</div>
                  </div>
                </Link>
              ...
              {nextYear ? (
                <Link
                  href={lp(locale, `/countdowns/ashura/${nextYear}`)}
                ...
                >
                  <div className="text-left">
                    ...
                    <div className="font-bold text-gray-800 dark:text-white">يوم عاشوراء {toArabicDigits(nextYear)}</div>
                  </div>
                  <span className="text-lg">{"\u2190"}</span>
                </Link>
              ) : <div className="flex-1" />}
            ...
            <div className="flex flex-wrap gap-3">
              <Link
                href={lp(locale, "/countdowns/ashura")}
                className="text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
              >
                {"\u2190"} العودة لعداد يوم عاشوراء
              </Link>
              <Link
                href={lp(locale, "/countdowns/ashura/years")}
                className="text-sm text-emerald-600 dark:text-emerald-400 hover:underline"
            ...
            <AdSlot id={`ashura-${year}-bottom`} size="leaderboard" />
          </div>
...
/* --- Info Cards --- */
function InfoCards({ data }: { data: AshuraYearData }) {
  return (
    <section className="grid grid-cols-2 sm:grid-cols-5 gap-3">
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-4 text-center">
        ...
        <div className="text-xs text-gray-400 mb-1">يوم عاشوراء</div>
        <div className="font-bold text-gray-800 dark:text-white text-sm">{formatDateAr(data.ashuraDate)}</div>
      </div>
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-4 text-center">
        <span className="text-2xl mb-2 block">📿</span>
        <div className="text-xs text-gray-400 mb-1">تاسوعاء</div>
        <div className="font-bold text-gray-800 dark:text-white text-sm">{formatDateAr(data.tasua)}</div>
      </div>
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-4 text-center">
        <span className="text-2xl mb-2 block">📅</span>
        <div className="text-xs text-gray-400 mb-1">١١ محرم</div>
        <div className="font-bold text-gray-800 dark:text-white text-sm">{formatDateAr(data.eleventhDate)}</div>
      </div>
      <div className="bg-white dark:bg-dark-surface rounded-2xl border border-gray-200 dark:border-gray-700 p-4 text-center">
...
/* --- Hero Component --- */
function YearHero({ data }: { data: AshuraYearData }) {
  const [time, setTime] = useState(getTimeUntilAshura(data.year));

  useEffect(() => {
    const interval = setInterval(() => {
      setTime(getTimeUntilAshura(data.year));
    }, 1000);
  ...
  return (
    <div className={`relative overflow-hidden ${
      isPast
        ...
        : "bg-gradient-to-bl from-emerald-600 via-teal-700 to-teal-800"
    }`}>
      {/* Decorative elements */}
      {!isPast && (
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14 text-center" dir="rtl">
        {/* Year title */}
        <div className="mb-2">
          <span className="inline-block bg-white/10 backdrop-blur-sm border border-white/15 rounded-full px-4 py-1.5 text-sm text-emerald-100">
            {data.content.heroSubtitle}
          </span>
        </div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-2">
          {isPast ? "يوم عاشوراء" : "🕌 العد التنازلي ليوم عاشوراء"} {toArabicDigits(data.year)}
        </h1>
        <p className="text-emerald-100/80 text-sm sm:text-base mb-6">
          {toArabicDigits(data.hijriYear)} هـ &middot; ١٠ محرم &middot; {formatDateAr(data.ashuraDate)}
        </p>

        {/* Countdown or Status */}
        {isPast ? (
          <div className="countdown-glass inline-block px-8 py-4">
            <p className="text-white/80 text-lg">انتهى يوم عاشوراء {toArabicDigits(data.year)}</p>
            <p className="text-white/50 text-sm mt-1">يوم نجّى الله فيه موسى عليه السلام</p>
          </div>
        ) : (
...
/* --- Year FAQ --- */
function YearFAQ({ data, year }: { data: AshuraYearData; year: number }) {
  const [openIndex, setOpenIndex] = useState<number | null>(null);

  const baseFAQ = [
    {
      q: `متى يوم عاشوراء ${toArabicDigits(year)}؟`,
      a: `يوم عاشوراء ${toArabicDigits(year)} (${toArabicDigits(data.hijriYear)} هـ) يوافق ١٠ محرم، ${formatDateAr(data.ashuraDate)}. وتاسوعاء في اليوم السابق ${formatDateAr(data.tasua)}.`,
    },
    {
      q: `ما فضل صيام يوم عاشوراء؟`,
      a: `صيام يوم عاشوراء يكفّر ذنوب سنة ماضية كما في صحيح مسلم عن النبي ﷺ. ويُستحب صيام تاسوعاء (٩ محرم) معه لمخالفة أهل الكتاب.`,
    },
    {
      q: `ما هي مراتب صيام عاشوراء؟`,
      a: `أفضلها صيام التاسع والعاشر والحادي عشر، ثم صيام التاسع والعاشر، ثم صيام العاشر وحده. وكلها مراتب فاضلة في صيام عاشوراء.`,
    },
    {
      q: `لماذا نصوم يوم عاشوراء؟`,
      a: `لأنه اليوم الذي نجّى الله فيه موسى عليه السلام وقومه من فرعون، فصامه النبي ﷺ شكراً لله وأمر بصيامه.`,
    },
  ];
  ...
  return (
    <section>
      ...
      />
      <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">
        أسئلة شائعة عن يوم عاشوراء {toArabicDigits(year)}
      </h2>
